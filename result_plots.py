...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='One of box, point, dist, violin')
    parser.add_argument(
        '-v', '--json_vars', type=str, nargs='+')
...
def main():
    ...
    if args.plot_type == 'box':
        ...
        xlabel = [str(x) for x in range(1, len(json_files) + 1)]
        pu.violinplot(json_files, args.json_vars[0], xlabel, args.out_file)
    ...
    elif args.plot_type == 'cumul_dist':
        pu.cumul_plot(json_files, args.iterations, args.out_file)
    elif args.plot_type == 'cumul_point':
        pu.cumul_point_plot(json_files, args.out_file)
